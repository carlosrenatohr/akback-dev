    ...
    <div style="margin-bottom: 0; background-color: rgba(50, 50, 50, 0.1); height:40px; width:100%; padding-top:3px;">
		...
		<div style="float:left; padding:2px;">
        	<a class="btn btn-default" style="outline:1;font:bold;" >
               <h2>Payments Received Report</h2>
            </a>
		</div>
</div>
...
<script type="text/javascript">
	...
	*/
	...
	function get_details(){
		...
		$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/payments_select',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
		 ...
		 var theme = getDemoTheme();
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
					{ text: 'Location', dataField: 'LocationName', width: '10%', filtertype: 'checkedlist',align: 'left',cellsalign: 'left' },
					{ text: 'Station', dataField: 'StationName', width: '10%', filtertype: 'checkedlist',align: 'left',cellsalign: 'left' },
					{ text: 'User', dataField: 'User', width: '10%',filtertype: 'checkedlist',align: 'left',cellsalign: 'left' },
					{ text: 'Date', dataField: 'CreatedDate', filtertype: 'input',cellsformat: 'MM/dd/yy hh:mm tt',width: '10%',filtertype: 'input',align: 'left',cellsalign: 'left'},					
					...
					{ text: 'Total', dataField: 'ReceiptTotal', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right'},
                     { text: 'Tendered', dataField: 'PayAmount', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
					 ...
					 { text: 'Received', dataField: 'PayApply', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					 ...
					 { text: 'Method', dataField: 'PayMethod', filtertype: 'checkedlist',width: '10%',align: 'right',cellsalign: 'right'},
						]
            });
	...
	$(document).ready(function(){
		...
		$("#DownloadExcel").click(function(){
			...
			$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/payments_export',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
				$('body').block({message: 'Loading...'})
			...
			success: function(data){
				$('body').unblock();
				console.log(data);	
				window.location = (window.location.origin + '/akback/backoffice/reports/payments_download');

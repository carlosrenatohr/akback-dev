...
<script type="text/javascript">
	...
	*/
	...
	function get_details(){
		...
		$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/tips_select',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
		 ...
		 //var url = window.location.origin + '/akback/backoffice/reports/payments_select/'+date_from+'/'+date_to;
			   ...
			   var source =
            {
                ...
                datafields: [
					...
					{ name: 'ReceiptSubTotal', type: 'number' },
					{ name: 'ReceiptTip', type: 'number' },
					{ name: 'ReceiptTax', type: 'number' },
					{ name: 'ReceiptTotal', type: 'number' },
					{ name: 'PayMethod', type: 'string' },
					{ name: 'Tendered', type: 'number' },
					{ name: 'Paid', type: 'number' },
					{ name: 'Change', type: 'number' },
					{ name: 'PaidTip', type: 'number' },
					{ name: 'PaymentDate', type: 'date',format:"yyyy-MM-dd"},
					{ name: 'PaymentUser', type: 'string' },
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
					...
					{ text: 'Order By', dataField: 'ReceiptUser', width: '10%', filtertype: 'input',align: 'left',cellsalign: 'left' },		
					{ text: 'SubTotal', dataField: 'ReceiptSubTotal',cellsformat: 'd2', width: '5%',align: 'right',cellsalign: 'right'},
					{ text: 'Tip', dataField: 'ReceiptTip', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right'},
                    { text: 'Tax', dataField: 'ReceiptTax', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right'},
					{ text: 'Total', dataField: 'ReceiptTotal', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right'},	
					{ text: 'Pay Method', dataField: 'PayMethod', width: '10%', filtertype: 'input',align: 'left',cellsalign: 'left' },
					{ text: 'Tendered', dataField: 'Tendered', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
					 ...
					 { text: 'Paid', dataField: 'Paid', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					...
					{ text: 'Paid Tip', dataField: 'PaidTip', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					...
					{ text: 'Change', dataField: 'Change', cellsformat: 'd2',width: '5%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					...
					{ text: 'Pay By', dataField: 'PaymentUser', filtertype: 'input',width: '10%',align: 'right',cellsalign: 'right'},
					{ text: 'Date', dataField: 'PaymentDate', cellsformat: 'MM/dd/yy',width: '10%',filtertype: 'input',align: 'left',cellsalign: 'left'}
						]
            });
	...
	$(document).ready(function(){
		...
		$("#DownloadExcel").click(function(){
			...
			$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/itemsales_export',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
				$('body').block({message: 'Loading...'})
			...
			success: function(data){
				...
				window.location = (window.location.origin + '/akback/backoffice/reports/itemsales_download');

...
<script type="text/javascript">
	...
	*/
	...
	function get_details(){
		...
		$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/categorysales_select',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
		 ...
		 var theme = getDemoTheme();
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
					...
					{ text: 'Category', dataField: 'Category', width: '10%', filtertype: 'checkedlist',align: 'left',cellsalign: 'left' },		
					{ text: 'Quantity', dataField: 'Quantity',cellsformat: 'd0', width: '5%',align: 'right',cellsalign: 'right',
					...
					{ text: 'List', dataField: 'ExtList', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
                    ...
                    { text: 'Discount', dataField: 'Discount', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					...
					{ text: 'Sell', dataField: 'ExtSell', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
					 ...
					 { text: 'Tax', dataField: 'Tax', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
	...
	$(document).ready(function(){
	 $("#pdfExport").click(function () {
                $("#jqxgrid").jqxGrid('exportdata', 'pdf', 'AK Category Sales Report');
            });
	})
	...
	$(document).ready(function(){
		...
		$("#DownloadExcel").click(function(){
			...
			$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/categorysales_export',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
				$('body').block({message: 'Loading...'})
			...
			success: function(data){
				$('body').unblock();
				console.log(data);	
				window.location = (window.location.origin + '/akback/backoffice/reports/categorysales_download');

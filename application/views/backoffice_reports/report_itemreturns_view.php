	...
	*/
		...
		var source =
            ...
            $("#jqxgrid").jqxGrid(
            {
                ...
                columns: [
					...
					{ text: 'Date', dataField: 'TransactionDate', cellsformat: 'MM/dd/yy hh:mm tt',width: '10%',filtertype: 'input',align: 'left',cellsalign: 'left'},
					...
					{ text: 'Total', dataField: 'ExtSellPrice', cellsformat: 'd2',width: '10%',align: 'right',cellsalign: 'right',
						aggregates: ['sum'], aggregatesrenderer: function (aggregates, column, element, summaryData) {
                          var renderstring = "<div class='jqx-widget-content jqx-widget-content-" + theme + "' style='float: left; width: 100%; height: 100%;'>";
                          $.each(aggregates, function (key, value) {
                              var name = key == 'sum' ? 'Sum' : 'Avg';
	...
	$(document).ready(function(){
		...
		$("#DownloadExcel").click(function(){
			...
			$.ajax({
			url: window.location.origin + '/akback/backoffice/reports/itemreturns_export',
			type: 'POST',
			data: postdata,
			beforeSend: function(){
				$('body').block({message: 'Loading...'})
